"""Create DatasetIR from Data.gov data, similar to World Bank middleware."""
...
def create_logical_ir_from_datagov_data(
    dataset_dir: Path,
    table_name: str = "main"
) -> LogicalIR:
    """
    Create LogicalIR from Data.gov CSV data.
    ...
    # Find CSV files (could be named "Comma_Separated_Values_File.csv" or other names)
    csv_files = list(dataset_dir.glob("*.csv"))
    
    if not csv_files:
        raise ValueError(f"No CSV files found in {dataset_dir}")
    
    # For Data.gov data, we'll use the first CSV file (usually there's just one)
    # If there are multiple, we'll combine them
    if len(csv_files) == 1:
        csv_file = csv_files[0]
        # Read a sample to infer schema (for large files)
        try:
            # Try reading first 1000 rows to infer schema
            df = pd.read_csv(csv_file, nrows=1000)
        except Exception:
            # If that fails, try reading the full file
    ...
    table = TableSpec(
        name=table_name,
        columns=columns,
        primary_key=[],  # Data.gov data typically doesn't have explicit PKs
        foreign_keys=[],
        constraints=[]  # No FDs for Data.gov data
    )
    
    # Create LogicalIR
    logical_ir = LogicalIR(
        tables={table_name: table},
        constraints=ConstraintSpec()  # No constraints for Data.gov data
    )
    
    return logical_ir


def create_dataset_ir_from_datagov_data(
    dataset_dir: Path,
    table_name: str = "main"
) -> DatasetIR:
    """
    Create a complete DatasetIR from Data.gov CSV data.
    ...
    Returns:
        DatasetIR instance
    """
    logical_ir = create_logical_ir_from_datagov_data(
...
if __name__ == "__main__":
    # Example: Create DatasetIR for traffic_data
    # Data is in data/datagov subdirectory
    data_dir = Path(__file__).parent.parent / "data" / "datagov"
    dataset_dir = data_dir / "traffic_data"
    
    if not dataset_dir.exists():
        print(f"Dataset directory not found: {dataset_dir}")
        print("Please run fetch_datagov_data.py first")
        sys.exit(1)
