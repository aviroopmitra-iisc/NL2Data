"""Create DatasetIR from OpenStreetMap data, similar to World Bank middleware."""
...
def create_logical_ir_from_osm_data(
    dataset_dir: Path,
    table_name: str = "main"
) -> LogicalIR:
    """
    Create LogicalIR from OpenStreetMap CSV data.
    ...
    # Find CSV files (OSM data typically has one CSV file)
    csv_files = list(dataset_dir.glob("*.csv"))
    
    if not csv_files:
        raise ValueError(f"No CSV files found in {dataset_dir}")
    
    # Use the first CSV file (usually there's just one for OSM data)
    csv_file = csv_files[0]
    
    # Read CSV file
    try:
        # Try reading first 1000 rows to infer schema (for large files)
    ...
    table = TableSpec(
        name=table_name,
        columns=columns,
        primary_key=[],  # OSM data typically doesn't have explicit PKs
        foreign_keys=[],
        constraints=[]  # No FDs for OSM data
    )
    
    # Create LogicalIR
    logical_ir = LogicalIR(
        tables={table_name: table},
        constraints=ConstraintSpec()  # No constraints for OSM data
    )
    
    return logical_ir


def create_dataset_ir_from_osm_data(
    dataset_dir: Path,
    table_name: str = "main"
) -> DatasetIR:
    """
    Create a complete DatasetIR from OpenStreetMap CSV data.
    ...
    Returns:
        DatasetIR instance
    """
    logical_ir = create_logical_ir_from_osm_data(
...
if __name__ == "__main__":
    # Example: Create DatasetIR for bengaluru_hospitals
    # Data is in data/openstreetmap subdirectory
    data_dir = Path(__file__).parent.parent / "data" / "openstreetmap"
    dataset_dir = data_dir / "bengaluru_hospitals"
    
    if not dataset_dir.exists():
        print(f"Dataset directory not found: {dataset_dir}")
        print("Please run fetch_osm_data.py first")
        sys.exit(1)
