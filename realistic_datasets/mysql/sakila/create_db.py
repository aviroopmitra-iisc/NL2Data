"""Create LogicalIR from Sakila database SQL schema and extract CSV files."""
...
def parse_create_table(sql_content: str) -> Dict[str, Dict]:
    ...
    # Pattern matches: CREATE TABLE ... ( ... ) ENGINE= or CREATE TABLE ... ( ... ) DEFAULT CHARSET or CREATE TABLE ... ( ... );
    create_table_pattern = r'CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?`?(\w+)`?\s*\((.*?)\)\s*(?:ENGINE\s*=|DEFAULT\s+CHARSET|;)'
    ...
    for match in matches:
        ...
        for part in parts:
            ...
            if re.match(r'PRIMARY\s+KEY', part, re.IGNORECASE):
                ...
                if pk_match:
                    pk_cols_str = pk_match.group(1) if pk_match.group(1) else ""
                    if pk_cols_str:
                        pk_cols = [c.strip().strip('`') for c in pk_cols_str.split(',')]
            ...
            # Skip KEY, INDEX, UNIQUE KEY, FULLTEXT KEY definitions
            if re.match(r'(KEY|INDEX|UNIQUE\s+KEY|FULLTEXT\s+KEY|SPATIAL\s+KEY)', part, re.IGNORECASE):
                continue
...
def create_logical_ir_from_sakila_sql(schema_file: Path) -> LogicalIR:
    """
    Create LogicalIR from Sakila SQL schema file.
    
    Args:
        schema_file: Path to sakila-schema.sql
...
if __name__ == "__main__":
    print("="*60)
    print("Sakila Database - Create LogicalIR and Extract CSV Files")
    print("="*60)
    
    # Get paths
    script_dir = Path(__file__).parent
    schema_file = script_dir / "sakila-schema.sql"
    data_sql_file = script_dir / "sakila-data.sql"
    base_dir = script_dir.parent.parent  # Go up to realistic_datasets/
    data_dir = base_dir / "data" / "mysql" / "sakila"
    data_dir.mkdir(parents=True, exist_ok=True)
    ...
    if not data_sql_file.exists():
        print(f"ERROR: {data_sql_file} not found")
        sys.exit(1)
    
    print(f"\nSchema file: {schema_file}")
    print(f"Data file: {data_sql_file}")
    ...
    print("="*60)
    logical_ir = create_logical_ir_from_sakila_sql(schema_file)
    ...
    print("="*60)
    tables_data = parse_insert_statements(data_sql_file)
